...
<main id="page-wrapper" class="page-wrapper">
    <div class="container contain-wrapper">
        ...
        {{-- Href Add Article --}}
        <a href="/dash/tips/article/add" class="btn btn-rounded btn-outline-primary mt-3">
        ...
        <div class="row row-cols-3 mt-3 row-filter">
            <div class="col">
                <input class="form-control" type="text" id="searchTipsByTitle" placeholder="Cari judul">
            </div>
        </div>
        {{-- Table --}}
        <table class="table tips-table table-striped table-hover mt-3">
            ...
            <tbody class="border border-primary text-center">
                ...
                <tr>
                    ...
                    <td>{{ $article->tipsCategory->category_name }}</td>
                    <td>{{ $article->creator }}</td>
                    <td>{{ $article->created_at }}</td>
                    <td>
                        <a href="{{ url('dash/tips/article/') }}/{{ $article->id }}"
                            class="btn btn-rounded btn-outline-warning">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ url('dash/tips/article/') }}/{{ $article->id }}" method="post"
                            class="d-inline">
                            @method('delete')
                            @csrf
