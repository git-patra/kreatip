...
<main id="page-wrapper" class="page-wrapper">
    <div class="container contain-wrapper">
        ...
        <button type="button" class="btn btn-rounded btn-outline-primary mt-3" data-toggle="modal"
            data-target="#add-course-materi"><i class="fas fa-plus"></i> Add</button>

        <!-- Add modal content -->
        <div id="add-course-materi" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body">
                        ...
                        <form action="{{ url('dash/materi/course') }}" method="post" class="pl-3 pr-3">
                            @csrf

                            <div class="form-group">
                                <label for="course">Course</label>
                                <input class="form-control" type="text" id="course" name="course" required
                                    placeholder="Course Materi" value="{{ old('course') }}">
                            </div>
        ...
        <table class="table table-striped table-hover mt-3">
            ...
            <tbody class="border border-primary text-center">
                ...
                <tr>
                    ...
                    <td>{{ $course->materiSubcategory->subcategory_name }}</td>
                    <td>{{ $course->creator }}</td>
                    <td>{{ $course->created_at }}</td>
                    <td>
                        <a href="{{ url('dash/materi/course/') }}/{{ $course->id }}"
                            class="btn btn-rounded btn-outline-warning">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ url('dash/materi/course/') }}/{{ $course->id }}" method="post"
                            class="d-inline">
                            @method('delete')
                            @csrf
