...
<main id="page-wrapper" class="page-wrapper">
    <div class="container contain-wrapper">
        ...
        <button type="button" class="btn btn-rounded btn-outline-primary mt-3" data-toggle="modal"
            data-target="#add-pelajar-info"><i class="fas fa-plus"></i> Add</button>

        <!-- Add modal content -->
        <div id="add-pelajar-info" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body">
                        ...
                        <form action="{{ url('dash/info/lain/pelajar') }}" method="post" class="pl-3 pr-3">
                            @csrf

                            <div class="form-group">
                                <label for="pelajar">Pelajar</label>
                                <input class="form-control" type="text" id="pelajar" name="pelajar" required
                                    placeholder="Pelajar" value="{{ old('pelajar') }}">
                            </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="custom-select mr-sm-2" id="subcategory" name="subcategory">
                                    ...
                                    @if ($subcategory->t_info_category_id === 1)
        ...
        <table class="table table-striped table-hover mt-3">
            ...
            <tbody class="border border-primary text-center">
                ...
                <tr>
                    ...
                    <td>{{ $pelajar->pelajar_name }}</td>
                    <td>{{ $pelajar->infoSubcategory->subcategory_name }}</td>
                    <td>{{ $pelajar->creator }}</td>
                    <td>{{ $pelajar->created_at }}</td>
                    <td>
                        <a href="{{ url('dash/info/lain/pelajar/') }}/{{ $pelajar->id }}"
                            class="btn btn-rounded btn-outline-warning">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ url('dash/info/lain/pelajar/') }}/{{ $pelajar->id }}" method="post"
                            class="d-inline">
                            @method('delete')
                            @csrf
