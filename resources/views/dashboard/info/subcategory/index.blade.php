...
<main id="page-wrapper" class="page-wrapper">
    <div class="container contain-wrapper">
        ...
        <button type="button" class="btn btn-rounded btn-outline-primary mt-3" data-toggle="modal"
            data-target="#add-subcategory-info"><i class="fas fa-plus"></i> Add</button>

        <!-- Add modal content -->
        <div id="add-subcategory-info" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body">
                        ...
                        <form action="{{ url('dash/info/subcategory') }}" method="post" class="pl-3 pr-3">
                            @csrf

                            <div class="form-group">
                                <label for="subcategory">Subcategory</label>
                                <input class="form-control" type="text" id="subcategory" name="subcategory" required
                                    placeholder="Subcategory Info" value="{{ old('subcategory') }}">
                            </div>
        ...
        <table class="table table-striped table-hover mt-3">
            ...
            <tbody class="border border-primary text-center">
                ...
                <tr>
                    ...
                    <td>{{ $subcategory->infoCategory->category_name }}</td>
                    <td>{{ $subcategory->creator }}</td>
                    <td>{{ $subcategory->created_at }}</td>
                    <td>
                        <a href="{{ url('dash/info/subcategory/') }}/{{ $subcategory->id }}"
                            class="btn btn-rounded btn-outline-warning">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ url('dash/info/subcategory/') }}/{{ $subcategory->id }}" method="post"
                            class="d-inline">
                            @method('delete')
                            @csrf
